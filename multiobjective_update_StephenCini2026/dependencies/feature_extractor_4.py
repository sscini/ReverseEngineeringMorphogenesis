...
class FeatureExtractor:
    ...
    def __init__(
        self, geometry_data, edge_data, n_squamous=20, n_columnar=100, n_cuboidal=10
    ):
    ...
    def tissue_basal_angle_variation(self):
        ...
        basal_center_x = self.vpos_x[
            self.n_total + (self.n_squamous + self.n_cuboidal + self.n_columnar) / 2
        ]
        basal_center_y = self.vpos_y[
            self.n_total + (self.n_squamous + self.n_cuboidal + self.n_columnar) / 2
        ]
        ...
        for i in range(int(self.n_columnar / 4)):
            ...
            pt1_x = self.vpos_x[
                (
                    self.n_total
                    + (self.n_squamous + self.n_cuboidal + self.n_columnar) / 2
                )
                + 1
                + 2 * i
            ]
