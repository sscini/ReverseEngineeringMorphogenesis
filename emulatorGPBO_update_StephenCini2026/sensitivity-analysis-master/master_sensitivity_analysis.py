...
BACKEND = get_backend(
    ...
    num_harmonics_efd=num_harmonic_efd,
)
...
xt_exp_basal, yt_exp_basal = spatial_efd.inverse_transform(
    coeffs_exp_basal, harmonic=num_harmonic_efd
)
...
for i in range(n_param_model):
    ...
    param_upper_bound = (
        paraminputs[param_index_model[i]] + 0.7 * paraminputs[param_index_model[i]]
    )
    param_lower_bound = (
        paraminputs[param_index_model[i]] - 0.7 * paraminputs[param_index_model[i]]
    )
    ...
    for j in range(2):
        ...
        archive_artifact(
            ...
            _script_path(
                os.path.join("output_data_files", "vertices_" + str(i) + "_" + str(j) + ".txt")
            ),
        )
