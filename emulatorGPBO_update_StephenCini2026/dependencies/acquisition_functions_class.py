...
class AcqisitionFunctions:
    ...
    def expected_improvement_modified(self, model_gpr, likelihood_gpr, f_target):
        ...
        with np.errstate(divide='warn'):
            ...
            bound_upper = np.divide(
                (f_target - model_predictions_mean + math.sqrt(error_best)),
                model_predictions_variance,
            )
            bound_lower = np.divide(
                (f_target - model_predictions_mean - math.sqrt(error_best)),
                model_predictions_variance,
            )
            ...
            ei_term_1_component_2 = error_best - np.square(
                f_target - model_predictions_mean
            )
            ...
            ei_term_2 = np.multiply(
                np.multiply(ei_term_2_component_1, ei_term_2_component_2),
                ei_term_2_component_3,
            )

            # ei_term_3_component_1 = [eps1*pdf(eps1) - eps2*pdf(eps2)]
            ei_term_3_component_1 = np.multiply(
                norm.pdf(bound_lower), bound_lower
            ) - np.multiply(norm.pdf(bound_upper), bound_upper)
            # ei_term_3_component_2 = [cdf(eps2/1.414) - cdf(eps1/1.414)]
            ei_term_3_component_2 = norm.cdf(bound_upper / math.sqrt(2)) - norm.cdf(
                bound_lower / math.sqrt(2)
            )
            ei_term_3_component_3 = ei_term_3_component_1 + ei_term_3_component_2 / 2
            # Taking square of model_predictions_variance
            ei_term_3_component_4 = np.multiply(
                model_predictions_variance, model_predictions_variance
            )
