...
# Path to the target shape data for which the parameters need to be estimated
...
dataPreprocess = DataPreprocessing(
    master_parameter_input_n, master_feature_output, num_samples
)
...
data_x = dataPreprocess.input_parameter_selection(
    num_parameters_LHS, LHS_parameter_index, master_parameter_input_log
)
...
xlimits = np.array(
    [
        [min_data_x[0], max_data_x[0]],
        [min_data_x[1], max_data_x[1]],
        [min_data_x[2], max_data_x[2]],
        [min_data_x[3], max_data_x[3]],
        [min_data_x[4], max_data_x[4]],
        [min_data_x[5], max_data_x[5]],
        [min_data_x[6], max_data_x[6]],
    ]
)
...
for i in range(n_iterations):
    ...
    model, likelihood, train_x_t, train_y_t, test_x_t, test_y_pc = (
        gpr.GP_model_definition(ExactGPModel, 0, 1)
    )
    # Traing the GPR model
    model, likelihood, lengthscale_hyperparameters = gpr.GP_training(
        train_x_t, train_y_t, model, likelihood, optimizer_type, num_iteration_gpr
    )
    ...
    ei, model_prediction_mean, model_prediction_variance = af.expected_improvement(
        model, likelihood, exploration_param_val
    )
    ...
    x_sampled = np.exp(
        np.add(
            np.multiply(x_sampled_logscale_standardized, data_x_variance), data_x_mean
        )
    )
    ...
    xt_sampled, yt_sampled = spatial_efd.inverse_transform(
        efd_coeff_sampled, harmonic=num_harmonics_efd
    )
    ...
    error_plot_path = os.path.join(run_time_folder_error, filename_error_iteration)
